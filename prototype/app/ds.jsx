// ── Kaani · Design System ─────────────────────────────────────
// Tokens, icon set, and shared atoms. Exports to window.
...
function Icon({ name, size = 22, stroke = 1.8, fill = 'none', style = {} }) {
...
function FoodImg({ preset = 'buffet', label, blur = 0, round = 0, height = '100%', children, style = {} }) {
...
// ── Atoms ─────────────────────────────────────────────────────
function Chip({ icon, children, tone = 'cream', style = {} }) {
  const tones = {
...
function Btn({ children, kind = 'primary', icon, iconR, full, size = 'md', onClick, style = {} }) {
...
// portions-left meter
function Portions({ left, total = 12, dark = false }) {
...
function Price({ from, original, size = 'md', light = false }) {
...
function SectionHead({ kicker, title, action, onAction, icon, accent = K.gold }) {
  return (
...
// little wordmark
function Wordmark({ size = 26, light = false }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
...
Object.assign(window, {
  ...
  Icon, FoodImg, FOOD_PRESETS, Chip, Btn, Portions, Price, SectionHead, Wordmark,
});
